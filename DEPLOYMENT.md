...
Há também um agendamento independente em `.github/workflows/news-sync.yml`, útil quando o Blueprint do Render não provisiona o Cron Job. Ele usa o segredo de repositório `NEWS_DATABASE_URL`, que deve apontar para o **mesmo PostgreSQL do Vercel**, com a URL externa do Render e SSL. O workflow pode ser iniciado manualmente em **Actions → Atualizar notícias → Run workflow**. O agendamento do GitHub usa UTC e pode sofrer atrasos de fila; o filtro de data e a seleção do ciclo continuam no código. Não grave a URL no repositório. Se o Cron Job do Render também estiver ativo, a trava no PostgreSQL e a unicidade das URLs evitam duplicação.
